#!/usr/bin/env python3
"""
Fix the job status endpoint in the API file with exact pattern matching.

This script adds proper error handling to the job status endpoint with the exact pattern that includes
...
def fix_job_status_endpoint():
    ...
    # Find the job status endpoint using the exact pattern with response_model
    endpoint_pattern = re.compile(
        r'@app\.get\("/jobs/\{job_id\}", response_model=JobResponse\).*?async def get_job_status.*?return job', 
        re.DOTALL
    )
    match = endpoint_pattern.search(content)
    
    if not match:
        print("Error: Could not find job status endpoint with exact pattern")
        return False
...
if __name__ == "__main__":
    print("Fixing the job status endpoint with exact pattern matching...\n")
    ...
    if fix_job_status_endpoint():
        ...
        print("\nFailed to fix job status endpoint. See error messages above.")
        sys.exit(1)
